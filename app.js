...
async function initApp() {
    ...
    dirtyPlateBtn.addEventListener('click', addDirtyPlate);
...
async function displayResults() {
    ...
    try {
        ...
        const { data: cleanPlatesWithNames, error: platesError } = await supabaseClient
            .from('plates')
            .select('name')
        ...
        // Get a random clean plate with name
        let randomCleanPlateText = '';
        if (cleanPlatesWithNames && cleanPlatesWithNames.length > 0) {
            // Select a random clean plate with name
            const randomIndex = Math.floor(Math.random() * cleanPlatesWithNames.length);
            const randomName = cleanPlatesWithNames[randomIndex].name;
            
            if (randomName) {
                randomCleanPlateText = `
                    <p class="clean-plate-highlight">
                        <i class="fas fa-trophy"></i> 
                        <span>${randomName}</span> won the Golden Plate Award! 
                        <i class="fas fa-award"></i>
                    </p>
                    <p class="award-description">For outstanding achievement in keeping their plate clean!</p>`;
            }
        } else if (currentSession.cleanPlates > 0) {
            // If there are clean plates but none with names
            randomCleanPlateText = `
                <p class="clean-plate-highlight">
                    <i class="fas fa-award"></i> 
                    We had ${currentSession.cleanPlates} clean ${currentSession.cleanPlates === 1 ? 'plate' : 'plates'}, but no names were recorded. 
